...
def load_raw() -> pd.DataFrame:
    ...
    if not path.exists():
        raise FileNotFoundError(f"Données absentes : {path}. Lance d'abord l'ingestion (readmission-ingest).")
...
def load_clean() -> pd.DataFrame:
    ...
    if not path.exists():
        raise FileNotFoundError(f"Données nettoyées absentes : {path}. Lance d'abord le nettoyage (readmission-clean).")
    return pd.read_parquet(path)
